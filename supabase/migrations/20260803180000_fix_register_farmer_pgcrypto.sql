-- register_farmer runs with search_path = public, so unqualified
-- gen_random_bytes() fails (pgcrypto lives in extensions on Supabase).
-- Qualify the call so farmer registration works with the anon key.
-- Execute grants for anon/authenticated are reaffirmed (already present).
