...
const PESTICIDE =
  /\b(registered|registration|approved (for|in)|pesticide register|what can i spray|what chemical|trade name|brand (to use|name)|label rate|pre-?harvest interval|re-?entry|PHI|REI|is .{0,30} legal (to (use|spray)|in))\b/i;

const PRODUCT_ASK =
  /\b(what (can|should) i (spray|use|buy)|what fungicide|what insecticide|what herbicide|what pesticide|product (for|to use)|available locally|local (product|chemical|spray))\b/i;
