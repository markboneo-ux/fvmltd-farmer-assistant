...
export function factsFromCase(record: CropCaseRecord): StructuredCaseFacts {
  return {
    ...
    farmingArea: record.farmingArea,
    latitude: record.latitude,
    longitude: record.longitude,
    growthStage: record.growthStage,
    symptomLocation: record.symptomLocation,
    onset: record.onset,
    spread: record.spread,
    percentageAffected: record.percentageAffected,
    recentRainfall: record.recentRainfall,
    forecastRainfall: record.forecastRainfall,
    temperature: record.temperature,
    humidity: record.humidity,
    photoFindings: record.photoFindings,
    diagnosticConfidence: record.diagnosticConfidence,
    missingInformation: record.missingInformation,
    suspectedPest: record.suspectedPest,
    suspectedDisease: record.suspectedDisease,
    confirmedDiagnosis: record.confirmedDiagnosis,
  };
}
...
export function buildNewCropCase(input: {
  ...
  return {
    ...
    onset: facts.onset,
    spread: facts.spread,
    percentageAffected: facts.percentageAffected,
    recentRainfall: facts.recentRainfall,
    forecastRainfall: facts.forecastRainfall,
    temperature: facts.temperature,
    humidity: facts.humidity,
    photoFindings: facts.photoFindings,
    diagnosticConfidence: facts.diagnosticConfidence,
    missingInformation: facts.missingInformation,
    suspectedPest: facts.suspectedPest,
    suspectedDisease: facts.suspectedDisease,
    confirmedDiagnosis: facts.confirmedDiagnosis,
    createdAt,
    updatedAt: createdAt,
  };
}
...
export function mergeUpdatedCase(
  ...
  return {
    ...
    farmingArea: merged.farmingArea ?? current.farmingArea,
    latitude: merged.latitude ?? current.latitude,
    longitude: merged.longitude ?? current.longitude,
    growthStage: merged.growthStage ?? current.growthStage,
    symptomLocation: merged.symptomLocation ?? current.symptomLocation,
    onset: merged.onset ?? current.onset,
    spread: merged.spread ?? current.spread,
    percentageAffected: merged.percentageAffected ?? current.percentageAffected,
    recentRainfall: merged.recentRainfall ?? current.recentRainfall,
    forecastRainfall: merged.forecastRainfall ?? current.forecastRainfall,
    temperature: merged.temperature ?? current.temperature,
    humidity: merged.humidity ?? current.humidity,
    photoFindings: merged.photoFindings ?? current.photoFindings,
    diagnosticConfidence: merged.diagnosticConfidence ?? current.diagnosticConfidence,
    missingInformation: merged.missingInformation ?? current.missingInformation,
    suspectedPest: merged.suspectedPest ?? current.suspectedPest,
    suspectedDisease: merged.suspectedDisease ?? current.suspectedDisease,
    confirmedDiagnosis: merged.confirmedDiagnosis ?? current.confirmedDiagnosis,
    updatedAt: nowIso(),
  };
}
